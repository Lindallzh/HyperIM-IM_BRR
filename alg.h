...
private:
    ...
    //std::vector<std::pair<uint32_t, uint32_t>> _vertexLayer;
...
public:
    ...
    {
       // std::cout << "555555555Alg(const Graph& graph, TResult& tRes) : " << std::endl;
        _numV = _hyperGraph.get_nodes();
        _numE = _hyperGraph.get_edges();
        _vecOutDegree = std::vector<uint32_t>(_numV);
      //  std::cout << "Alg(const Graph& graph, TResult& tRes) : " << std::endl;
        for (auto &nbrs : graph)
        {
            for (auto &node : nbrs)
            {
