...
double Alg::subsimOnly(const int targetSize, const double epsilon, const double delta)
{
    ...
    //const auto numIter = (size_t)((log2(maxNumR / numRbase))/(_hyperGraph.get_pMax())) + 1;
    const auto numIter = (size_t)((log2(maxNumR / numRbase))/(_hyperGraph.get_pMax())) + 1;
    const double a1 = log(numIter * 3.0 / delta);
    const double a2 = log(numIter * 3.0 / delta);
    double time1 = 0.0, time2 = 0.0, time3 = 0.0;
 std::cout << "execute ALG:11111111111111111111"<< std::endl;
    std::cout << std::endl;
    for (auto idx = 1; idx <= numIter; idx++)
    {
       ...
       // _hyperGraph.BuildRRsets(numR); // R1
        _hyperGraph.BuildRRsetsR2(numR);
        ...
        //  const auto infSelf =_hyperGraph.CalculateInf(_vecSeed); 
       // _hyperGraph.BuildRRsetsR2(numR);
         //timerSubsim.log_operation_time();
       
       // _hyperGraphVldt.BuildRRsetsR2(numR);
        ...
        if (currApprox >= approx - epsilon)
        {
            ...
            double avgSize = _hyperGraph.HyperedgeAvg()+_hyperGraphVldt.HyperedgeAvg();
            size_t RR_size= _hyperGraph.get_RR_size()+_hyperGraphVldt.get_RR_size();
             _hyperGraph.display_hyperedge_stat();
             _hyperGraphVldt.display_hyperedge_stat();
            std::cout <<"Sampling number: " << RR_size << '\n';
            std::cout <<"totalSamples: " << avgSize << '\n';
            std::cout << "==>Influence via R2: " << infVldt << ", time: " << _res.get_running_time() << '\n';
            std::cout << "==>Time for RR sets and greedyCNT: " << time1 << ", " << time2 << '\n';
            return 0;
        }
    }
...
double Alg::FindRemSet(const int targetSize, const double epsilon, const double targetEpsilon, const double delta)
{
    ...
    for (auto idx = 1; idx <= numIter; idx++)
    {
        ...
        if (currApprox >= approx - targetEpsilon)
        {
            ...
            _res.set_seed_vec(_vecSeed);
            _res.set_RR_sets_size(_numRRsets * 2);
             std::cout << "==>fullRRSize: " << fullRRSize  << std::endl;
...
double Alg::FindDynamSub(const int totalTargetSize, const double epsilon, const double delta)
{
    ...
    for (auto idx = 1; idx <= numIter; idx++)
    {
        ...
        for (int i = _vecSeed.size() - 1; i >= 0; i--)
        {
            ...
            double p_max=_hyperGraph.getPMax(i);
            double p_min=_hyperGraph.getPMin(i);
               
                 
              if(p_min!=p_max)
              {
                 calcAppr = (1-exp(1))/(1-pow((1-p_max)/p_max,100)-epsilon);
              }
              else
              {
       ...
       //added by llzh 2023-1-1
       // const double approx = 1 - pow(x, _vecSeed.size());

        //get_pMax();
       double p_max_m=_hyperGraph.get_pMax();
       const double approx = 1 - pow(x, _vecSeed.size());
       
      // const double approx = (1-exp(1))/(1-pow((1-p_max_m)/p_max_m, _vecSeed.size()));
...
double Alg::subsimWithHIST(const int targetSize, const double epsilon, const double delta)
{
    ...
    double fullRRSize = _hyperGraph.HyperedgeAvg();
    size_t RR_size_=_hyperGraph.get_RR_size();
     _hyperGraph.display_hyperedge_stat();
    std::cout << "==>fullRRSize: " << fullRRSize  << std::endl;
    std::cout << "==>RR_size_: " << RR_size_  << std::endl;
    _res.set_running_time(timerSubsim.get_total_time());
    return 0.0;
}
